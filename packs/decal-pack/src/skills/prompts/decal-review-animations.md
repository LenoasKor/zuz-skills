---
id: decal-review-animations
label: 데칼 애니메이션 리뷰 · review-animations
version: 1.0.0
risk: read_only
group: Motion
target: active-cli
...
- Review only the requested diff or motion surface. Do not edit files, register work, commit, or turn findings into implementation without a separate user request.
- Decal is a high-frequency professional tool, so crisp, responsive, restrained motion is the product default. A frequently repeated interaction has a smaller motion budget than an occasional or first-run surface.
- Respect documented project decisions and existing motion tokens. Report a conflict with those decisions rather than silently replacing them with the upstream default.
- Load the allowlisted `STANDARDS.md` companion only when an exact curve, duration, spring, gesture, performance, or accessibility value is needed: `decal_ui.read_canonical_skill({ skillId: "decal-review-animations", resourcePath: "STANDARDS.md" })`.
- If the resource tool is unavailable, use the standards present in this document and mark any exact value you could not verify instead of guessing.
