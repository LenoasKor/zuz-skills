...
- 외부 host의 일반 커밋은 호스트 정책과 유효한 기존 사전·자동 승인 또는 현재 요청에 따라 정확한 범위를 보고한 뒤 수행하며, 유효한 승인이 있으면 재승인을 요구하지 않습니다. 등록 digest 승인과 Native 경계는 유지합니다. 완료·정산의 명시 요청은 그 작업이 발급한 exact settlement write-set의 즉시 격리 커밋과 `npm run settlement:finalize`까지 같은 승인으로 연속 수행합니다.
