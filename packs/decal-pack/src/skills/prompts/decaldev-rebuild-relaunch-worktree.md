...
프로젝트가 `출시 전`이든 `운영 중`이든 linked worktree의 일반 구현·Smoke·기능 확인은 이 isolated_test lane을 사용합니다. 운영 중 공식 QA나 최종 main 머지 후 검증은 이 스킬이 아니라 main의 릴리즈 후보 경로로 전환합니다.
