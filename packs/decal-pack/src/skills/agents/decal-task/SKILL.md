...
- Outside Decal, use `contracts/task-work-bug/v6/register-task-batch.mjs` for both single and multi-Task registration. First run it with the complete semantic intent and `--dry-run`; show the candidate Tasks, `intentDigest`, provisional assigned identities, exact write-set, and the fact that approval includes one exact registration commit. Only when the current user turn approves that semantic scope, rerun with the same intent, `--approved-digest ...`, and `--write`. The writer acquires the shared repository lock, recomputes final IDs on latest `main`, validates, writes, and commits the exact registration paths. It does not authorize push, merge, deploy, lifecycle completion, or version settlement.
...
- If either the validator or v6 writer is unavailable, stop and report `authoring_update_required` instead of extending the current index by inspection or relying on a network gateway. A repository explicitly pinned to v4 may keep its legacy file-only writer, but do not describe that path as atomic registration or reuse a v6 approval for a later manual commit.
