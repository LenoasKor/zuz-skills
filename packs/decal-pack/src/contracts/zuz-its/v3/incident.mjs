...
export function parseIncident(source, fileName = null) {
  ...
  if (typeof id !== "string" || !/^INC-[0-9]+$/u.test(id) || /^INC-0+$/u.test(id) || (fileName && fileName !== `${id}.md`)) fail("identity_invalid");
...
export async function scanIncidents(root) {
  ...
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    if (!/^INC-[0-9]+\.md$/u.test(entry.name)) continue;
    ...
    const record = parseIncident(source, entry.name);
    const identity = canonicalIncidentId(record.id);
...
export function nextIncidentId(highest) {
  ...
  return `INC-${String(next)}`;
}

export function canonicalIncidentId(id) {
  const match = /^INC-([0-9]+)$/u.exec(String(id ?? ""));
  if (!match || BigInt(match[1]) === 0n) fail("identity_invalid");
  return `INC-${BigInt(match[1])}`;
}

export async function resolveIncident(root, requestedId) {
  const canonicalId = canonicalIncidentId(requestedId);
  const scan = await scanIncidents(root);
  const matches = scan.records.filter((record) => canonicalIncidentId(record.id) === canonicalId);
  if (matches.length === 0) fail("incident_not_found");
  if (matches.length !== 1) fail("duplicate_record_identity");
  const record = matches[0];
  const relative = canonicalRelative(record.id);
  return { record, relative, target: await assertPlainPath(root, relative), canonicalId };
}
