...
import { canonicalIncidentId, fail, INCIDENT_FLOW, listValues, output, parseArgs, parseIncident, resolveIncident, sourceRevision } from "./incident.mjs";
...
try {
  ...
  if (![...INCIDENT_FLOW, "blocked"].includes(targetStatus)) fail("usage_error");
  const resolved = await resolveIncident(root, id);
  const relative = resolved.relative;
  const target = resolved.target;
  const source = await (await import("node:fs/promises")).readFile(target, "utf8");
  const record = parseIncident(source, `${resolved.record.id}.md`);
  ...
  if (targetStatus === "closed") {
    ...
    if (reason === "duplicate" && canonicalIncidentId(String(args.get("--duplicate-of") || "")) === resolved.canonicalId) fail("duplicate_reference_required");
  ...
  const verified = parseIncident(nextSource, `${resolved.record.id}.md`);
  await atomicReplace(target, nextSource);
  output(RESULT_SCHEMA, { status: "written", code: "accepted", id: resolved.canonicalId, previousStatus: record.status, currentStatus: verified.status, previousSourceRevision: record.revision, sourceRevision: sourceRevision(nextSource), writeSet: [relative] });
} catch (error) {
