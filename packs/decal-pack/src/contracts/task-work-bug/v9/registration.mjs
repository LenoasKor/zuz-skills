...
const INTENT_SCHEMA = "decal.zuz-its.ticket-registration-intent/v9";
const RESULT_SCHEMA = "decal.zuz-its.ticket-registration/v9";
const PREDECESSOR_MANIFEST_SHA256 = "5a15801488dde89b0fa187cb9dcdeffa7f8fbb73fcb9a7c9bb2aee7eeca62957";
...
async function validatePackage(root) {
  const predecessor = path.join(root, "contracts/task-work-bug/v8/manifest.json");
  ...
  if (manifest?.schema !== "decal.task-work-bug.fixture-manifest/v9" || !manifest.files) fail("contract_digest_mismatch");
...
function recordIdentity(kind, source) {
  ...
  if (!new RegExp(`^${definition.prefix}[0-9]+$`, "u").test(id ?? "") || /-0+$/u.test(id)) fail("invalid_identity");
  return id;
}
...
async function scanKind(root, kind) {
  ...
  let maximum = "0";
  const pattern = new RegExp(`^${definition.prefix}([0-9]+)\\.md$`, "u");
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    const identity = match[1].replace(/^0+/u, "");
    if (!identity) fail("invalid_identity");
    if (identities.has(identity)) fail("duplicate_record_identity");
...
async function receipt(root, intentDigest, commit, status = "replayed") {
  ...
  if (paths.length !== 1 || !/^docs\/work-items\/(?:work|bugs|incidents)\/(?:WORK|BUG|INC)-[0-9]+\.md$/u.test(paths[0])) fail("invalid_replay_commit");
  const source = String(await git(root, ["show", `${commit}:${paths[0]}`]));
  const id = /^id:\s*((?:WORK|BUG|INC)-[0-9]+)$/mu.exec(source)?.[1];
...
export async function recoverTicketRegistration(root) {
  ...
  if (
    ...
    || !/^docs\/work-items\/(?:work|bugs|incidents)\/(?:WORK|BUG|INC)-[0-9]+\.md$/u.test(value.path)
...
export async function registerTicket({ root, intent: rawIntent, approvedDigest, testFailAt = null }) {
  ...
  try {
    ...
    const id = `${KINDS[intent.kind].prefix}${number}`;
