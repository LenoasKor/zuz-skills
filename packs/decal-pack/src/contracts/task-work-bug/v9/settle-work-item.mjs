...
export async function planWorkItemSettlement({ root, kind, id }) {
  ...
  const canonicalId = canonicalTicketId(kind, id);
  ...
  const record = parseWorkBug(source, kind, resolved.id);
  ...
  const plan = {
    root, baseHead, operation: `${kind}-settlement`, recordId: canonicalId, message: `chore(its): settle ${canonicalId}`,
