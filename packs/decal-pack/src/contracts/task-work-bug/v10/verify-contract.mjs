...
const required = ["README.md", "contract.json", "main-authority.mjs", "register-task-batch.mjs", "register-ticket.mjs", "verify-contract.mjs"];
...
export async function verifyContract(directory = path.dirname(fileURLToPath(import.meta.url))) {
  ...
  if (manifest.schema !== "decal.task-work-bug.fixture-manifest/v10" || !manifest.files || Array.isArray(manifest.files)
  ...
  const dependencies = ["v7/manifest.json", "v9/manifest.json"];
