...
const JOURNAL_FILE = "task-registration-pending-v7.json";
const ABSENT_DIGEST = "absent";
const INTENT_SCHEMA = "decal.task-work-bug.task-registration-batch-intent/v7";
const RESULT_SCHEMA = "decal.task-work-bug.task-registration-batch/v7";
...
async function validateV7Package(root) {
  const packageRoot = path.join(root, "contracts/task-work-bug/v7");
  const predecessor = await plainFile(path.join(root, "contracts/task-work-bug/v6/manifest.json"));
  ...
  if (manifest?.schema !== "decal.task-work-bug.fixture-manifest/v7" || !manifest.files) {
...
function normalizeDependency(value) {
  ...
  if (value && typeof value === "object" && Number.isSafeInteger(value.taskId) && value.taskId > 0) {
...
async function readJournal(root) {
  ...
  if (journal?.schema !== "decal.task-work-bug.task-registration-journal/v7" || !Array.isArray(journal.entries)) {
    fail("invalid_journal");
  }
  const validDigest = (value, allowAbsent = false) => (
    (allowAbsent && value === ABSENT_DIGEST)
    || (typeof value === "string" && /^sha256:[0-9a-f]{64}$/u.test(value))
  );
  const seen = new Set();
  if (
    journal.entries.length < 3
    || journal.entries.length > 258
    || typeof journal.intentDigest !== "string"
    || !/^sha256:[0-9a-f]{64}$/u.test(journal.intentDigest)
    || journal.entries.some((entry) => {
      const validPath = entry?.path === "docs/tasks/index.md"
        || entry?.path === "docs/tasks/category_index.md"
        || /^docs\/tasks\/task_[1-9][0-9]*_[a-z0-9_]+\.md$/u.test(entry?.path ?? "");
      const duplicate = seen.has(entry?.path);
      seen.add(entry?.path);
      return !validPath
        || duplicate
        || !(entry.before === null || typeof entry.before === "string")
        || !validDigest(entry.beforeDigest, true)
        || !validDigest(entry.afterDigest);
    })
  ) fail("invalid_journal");
  return journal;
}
...
export async function registerTaskBatch({ root, intent: rawIntent, approvedDigest, testFailAt = null }) {
  ...
  try {
    ...
    const journal = {
      schema: "decal.task-work-bug.task-registration-journal/v7",
      phase: "preparing",
