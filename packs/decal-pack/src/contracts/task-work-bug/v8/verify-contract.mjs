...
const required = ["README.md", "finalize.mjs", "recover.mjs", "settle-work-item.mjs", "strict-json.mjs", "task-lifecycle.mjs", "task-policy.mjs", "transaction.mjs", "verify-contract.mjs", "version-profile.mjs", "work-bug-policy.mjs", "work-item-lifecycle.mjs"];
...
export async function verifyContract(directory = path.dirname(fileURLToPath(import.meta.url))) {
  ...
  if (manifest.schema !== "decal.task-work-bug.fixture-manifest/v8" || !manifest.files || Array.isArray(manifest.files)
  ...
  if (!manifest.dependencies || JSON.stringify(Object.keys(manifest.dependencies)) !== JSON.stringify(["v7/default-branch.mjs"])) fail("invalid_contract_dependencies");
