...
export function transitionWorkItem(source, { kind, id, target, timestamp, summary, evidence, reason, note, duplicateOf }) {
  ...
  if (target === "closed") {
    ...
    if (reason === "duplicate") {
      if (!new RegExp(`^${kind === "work" ? "WORK" : "BUG"}-[0-9]{3,}$`, "u").test(duplicateOf) || duplicateOf === id) fail("duplicate_reference_required");
...
export async function planWorkItemTransition({ root, expectedSourceRevision, ...request }) {
  ...
  if (!Object.hasOwn(FLOWS, kind) || !new RegExp(`^${kind === "work" ? "WORK" : "BUG"}-[0-9]{3,}$`, "u").test(id)) fail("invalid_record_identity");
  ...
  const result = transitionWorkItem(source, request);
  const plan = { root, baseHead, recordId: id, operation: `${kind}-transition`, message: `chore(its): ${id} ${request.target}`,
